...
```env
# API de OpenAI (solo si quieres traducción de SQL entre dialectos)
...
# Ocultar funcionalidades de ChartDB Cloud
VITE_HIDE_CHARTDB_CLOUD=true
```

> ⚠️ Si no configuras nada, la app funciona completamente — los datos se almacenan en tu navegador (IndexedDB). La IA es **opcional** y solo se usa para exportar SQL a otro dialecto.
...
```bash
# Build básico (sin analytics, sin IA)
docker build -t chatdb:local \
  --build-arg VITE_DISABLE_ANALYTICS=true \
  .

# Build con IA (OpenAI)
docker build -t chatdb:local \
  --build-arg VITE_OPENAI_API_KEY=sk-tu-key \
  --build-arg VITE_DISABLE_ANALYTICS=true \
  .

# Build con servidor LLM local
docker build -t chatdb:local \
  --build-arg VITE_OPENAI_API_ENDPOINT=http://localhost:11434/v1 \
  --build-arg VITE_LLM_MODEL_NAME=qwen2.5:32b \
  --build-arg VITE_DISABLE_ANALYTICS=true \
  .
```

> ⏱️ El primer build tarda ~3-5 minutos (descarga dependencias). Los siguientes son rápidos por el cache de layers.
...
# Con servidor LLM local (Ollama)
docker run -d \
  -p 8080:80 \
  --name chatdb \
  -e DISABLE_ANALYTICS=true \
  -e OPENAI_API_ENDPOINT=http://host.docker.internal:11434/v1 \
  -e LLM_MODEL_NAME=qwen2.5:32b \
  chatdb:local
```
...
docker rmi chatdb:local # Eliminar imagen
...
heroku container:push web \
  --app nombre-de-tu-app \
  --arg VITE_OPENAI_API_KEY=sk-tu-key \
  --arg VITE_DISABLE_ANALYTICS=true
```
...
```bash
heroku container:release web --app nombre-de-tu-app
```
...
A diferencia del build, las variables de runtime se configuran así:

```bash
# Desactivar analytics
heroku config:set DISABLE_ANALYTICS=true --app nombre-de-tu-app

# Con servidor LLM externo
heroku config:set OPENAI_API_ENDPOINT=https://tu-servidor/v1 --app nombre-de-tu-app
heroku config:set LLM_MODEL_NAME=tu-modelo --app nombre-de-tu-app
...
heroku container:push web --app nombre-de-tu-app --arg VITE_DISABLE_ANALYTICS=true
...
1. **No subas `.env` al repositorio** — está en `.gitignore`
2. **Credenciales de IA**: usa variables de entorno de Heroku (`heroku config:set`), nunca las hardcodees en el código
3. **Analytics**: está desactivado por defecto en los builds de Docker (`VITE_DISABLE_ANALYTICS=true`)
4. **Datos**: todo se almacena localmente en el navegador (IndexedDB/Dexie) — no hay base de datos en el servidor
...
- **App en Heroku**: https://chatdb-alber-060263da84d8.herokuapp.com/
...
- **Documentación Heroku Container Registry**: https://devcenter.heroku.com/articles/container-registry-and-runtime
