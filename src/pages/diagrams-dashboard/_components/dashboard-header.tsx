        ...
        <header className="flex h-16 select-none items-center justify-between border-b border-border/40 bg-card px-4 font-primary md:px-6">
            <div className="flex items-center gap-x-4">
                <img
                    ...
                    onClick={() => navigate('/')}
                />
                <Badge variant="secondary" className="rounded-md font-mono text-[10px] font-bold text-muted-foreground uppercase px-2 py-0.5 tracking-wider">
                    Dashboard
                </Badge>
            </div>
            ...
            <div className="flex items-center gap-x-3">
                ...
                {isAdmin && isAdmin() && (
                    <Button
                        variant="outline"
                        className="h-8 gap-1.5 border-primary/20 bg-primary/5 px-3 text-xs font-semibold text-primary transition-all duration-200 hover:bg-primary/10 hover:border-primary/30"
                                ...
                                <Avatar className="size-full">
                                    ...
                                    <AvatarFallback className="bg-primary/5 text-xs font-semibold text-primary">
                                        {profile?.display_name
                                            ?.substring(0, 2)
                                            .toUpperCase() || user.email?.substring(0, 2).toUpperCase() || 'US'}
                                    </AvatarFallback>
                                </Avatar>
                            </Button>
                        </DropdownMenuTrigger>
                        <DropdownMenuContent align="end" className="w-56">
                            <DropdownMenuLabel className="font-normal">
                                <div className="flex flex-col space-y-1">
                                    ...
                                    <div className="pt-1">
                                        <span className={`inline-block rounded-full px-2 py-0.5 text-[9px] font-semibold uppercase tracking-wider ${roleInfo.className}`}>
                                            {roleInfo.label}
                                        </span>
                                    </div>
                                </div>
